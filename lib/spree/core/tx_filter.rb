  ...
  module Core
    class TxFilter < Spree::Core::BaseFilter
      ...
      def self.brand_filter
        ...
        self.new(:brand, taxonomy.root.leaves.includes(:translations)).filter
      end

      def self.category_filter
        taxonomy = Spree::Taxonomy.find(Rails.configuration.category_id)
        self.new(:category, taxonomy.root.leaves.includes(:translations)).filter
      end

      def self.all_tx
        all_tx_filters = []
        all_tx_filters << self.brand_filter
        all_tx_filters << self.category_filter
        all_tx_filters
      end
    end
  end
